...
with open('dummy_data.txt') as f:
    for line in f:
        ...
        data.data.append((float(_x1), float(_x2), float(_y)))

shutil.rmtree('/tmp/tensorflow_1', ignore_errors=True)

# Automatically handles initialization, summary writing, etc.
with tf.train.MonitoredTrainingSession(
    ...
    while not sess.should_stop():
        ...
        sess.run(optimize_op, feed_dict={x: input_x, y: input_y})
